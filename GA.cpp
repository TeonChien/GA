...
#define mutation_num 5  //突變要換的點數量
using namespace std;
...
float group_center[group_num][dimension];     //群中心
...
void center_find(int arr[parent_num][data_num],int z) {
    for (int k = 0; k < group_num; k++) {
        ...
        for (int n = 0; n < dimension; n++) {
            group_center[k][n] = (total[n] / count);
        }
    }
}

float distance_cal(float total, int i, int j) {
    float distance = 0;
    for (int k = 0; k < dimension; k++) {
        distance += (Dataset[Groupset[i][j]][k] - group_center[i][k]) *
                    (Dataset[Groupset[i][j]][k] - group_center[i][k]);
    }
    total += distance;
    return total;
}
...
int parent_choice(const int *fitness, int fitness_total) {
    int index;
    int random_number=rand()%fitness_total;

    if(random_number>=0&&random_number<fitness[0]){
        index=0;
    }
    else if(random_number>=fitness[0]&&random_number<fitness[1]){
        index=1;
    }
    else if(random_number>=fitness[1]&&random_number<fitness[1]+fitness[2]){
        index=2;
    }
    else if(random_number>=fitness[1]+fitness[2]&&random_number<fitness[1]+fitness[2]+fitness[3]){
        index=3;
    }
    else if(random_number>=fitness[1]+fitness[2]+fitness[3]&&random_number<fitness[1]+fitness[2]+fitness[3]+fitness[4]){
        index=4;
    }
    else{
        index=5;
...
void initial() {
    for (int i = 0; i < parent_num; i++) {      //隨機產生母群的解
        for (int j = 0; j < data_num; j++) {
            parent[i][j] = rand() % group_num;
        }
    }
    cout<<"parent"<<endl;
    for (int z = 0; z < parent_num; z++) {     //產生母群的SSE
...
void crossover(){
    int rand_choice1 = rand()%parent_num;
    int rand_choice2;
    int temp[data_num];
    int count=0;
    for(int i=0;i<parent_num/2;i++) {
        do {
            rand_choice2 = rand() % parent_num;
        } while (rand_choice2 == rand_choice1);

        int change_point = rand() % data_num;
        if((double)(rand())/(RAND_MAX + 1.0)>0.05) {
            for (int j = change_point; j < data_num; j++) {
                temp[j] = choice[rand_choice1][j];
                choice[rand_choice1][j] = choice[rand_choice2][j];
                choice[rand_choice2][j] = temp[j];
            }
            for (int k = 0; k < data_num; k++) {
                children[i * 2][k] = choice[rand_choice1][k];
                children[i * 2 + 1][k] = choice[rand_choice2][k];
            }
        }
        else{
            for (int k = 0; k < data_num; k++) {
...
int main() {
    ...
    for(int i=0;i<circulartimes;i++) {
        ...
        cout << "children" << endl;
        for (int z = 0; z < parent_num; z++) {     //產生子群的SSE
